...
#Hash all the songs in the audio/ folder and store it as a pkl file
	...
	def hash_added(self):
		...
		for songname in sorted(os.listdir(add_songs_path)):
			if songname == '.DS_Store':
				continue
			cur_song = self.create_song(add_songs_path+songname, try_dumped=True)
			self.hash_one_song(cur_song, is_target=False)
			print("Hashed",cur_song.name)
			#os.remove(add_songs_path+songname)
			os.rename(add_songs_path+songname,self.audio_path+songname)
		with open(self.data_path+'database.pkl', 'wb') as f:
			pickle.dump(self.database, f)
